...
export default function ExercisePartLabel({ exercisePart, type, selectedPart }: Props) {
  return (
    <>
      {type === "part" && (
        <S.Label htmlFor={exercisePart}>
          ...
          />
          <S.PartLabel>{exercisePart}</S.PartLabel>
        </S.Label>
      )}
      {type === "exercise" && (
        <S.ExerciseLabel id={exercisePart}>{exercisePart}</S.ExerciseLabel>
      )}
    </>
  );
}
